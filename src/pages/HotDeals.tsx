...
const HotDeals = () => {
  ...
  const channelName = settings?.telegramChannel || '@VoeAVTO';
  const { posts, loading, error } = useTelegramChannel(channelName);
  const { toast } = useToast();
  const isMobile = useIsMobile();
  
  const navigateToTelegram = () => {
    const telegramUrl = `https://t.me/${channelName.replace('@', '')}`;
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Актуальные предложения из нашего Telegram канала {channelName}
            </p>
          </div>
          <Button 
        ...
        {loading ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-md p-6 text-center">
            ...
            <p className="text-gray-500 mt-2">
              Технические детали ошибки: {error}
            </p>
          </div>
        ) : (
